...
int main(int argc, char **argv) {
    ...
    const char *library_path = getenv("ANDROID_MIHOMO_LIBRARY");
    if (library_path) {
        if (library_path[0] != '/') {
            fprintf(stderr, "ANDROID_MIHOMO_LIBRARY must be an absolute path\n");
            return 64;
        }
    } else {
    ...
    /* Import libc environment before Mihomo package initializers run. */
    if (setenv("ANDROID_MIHOMO_CLI", "1", 1) != 0) return 70;
    void *library = dlopen(library_path, RTLD_NOW | RTLD_LOCAL);
    if (!library) {
        dprintf(saved_err, "Cannot load Mihomo library %s: %s\n", library_path, dlerror());
        return 70;
    }
    cli_entry entry = (cli_entry)dlsym(library, "AndroidMihomoCLI_v1");
    if (!entry) {
        dprintf(saved_err, "Incompatible Mihomo library (missing CLI ABI v1): %s\n", dlerror());
        return 71;
    }
